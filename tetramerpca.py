...
def main(filename, outdir, window, step, blast_cmd, db, num_threads,
    ...
    P = PCA(DF)
    tetra['P'] = P
    # we don't save PCA (even as a pickle) because it it limited in value and big
    #ofile = os.path.join(tetra['outdir'], ''.join([tetra['name'], '-PCA.pickle']))
    #ok = inout.write_PCA(tetra['P'], filename = ofile)

    # but the fracs are the % explanations used in the plots
    tetra['fracs'] = draw.get_explanation(tetra)
    ofile = os.path.join(tetra['outdir'], ''.join([tetra['name'], '-fracs.pickle']))
    ok = inout.write_fracs(tetra['fracs'], filename = ofile)
    ...
    ofile = os.path.join(tetra['outdir'], ''.join([tetra['name'], '-outliers.csv']))
    ok = inout.write_outliers_table(tetra['outliers'], filename = ofile)
